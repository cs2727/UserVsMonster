...
const alertMessages = [
  ...
  'Unfortunately, the Monster has defeated you, better luck next time!',
];
...
function userInput(eventObject) {
  ...
  if (idOfButton === 'attack-Button') {
    ...
    value = Math.floor(Math.random() * (20 - 15 + 1) + 15);
  } else {
  ...
  attackOrHeal === 0
    ? changingAudio('Audio/GameAudio/userAttack.mp3')
    : changingAudio('Audio/GameAudio/healing.mp3');
  attackOrHeal === 0
    ? (monsterHealthBar.value -= value)
    : (userHealthBar.value += value); // User attacks Monster or Heals
  attackOrHeal === 0
    ? (monsterHealthNumber.innerText = monsterHealthBar.value)
    : (userHealthNumber.innerText = userHealthBar.value);
...
function monsterResponse() {
  ...
  const intensityMax = roundCounter === 1 ? 15 : roundCounter === 2 ? 20 : 25;
  const intensityMin = roundCounter === 1 ? 10 : roundCounter === 2 ? 18 : 20;
  let value; // the value of the attack or heal

  if (monsterResponseToggler === false) {
    //Stops Monster from responding to User if Monster is dead
  ...
  if (attackOrHeal === 1) {
    ...
    value = Math.floor(
      Math.random() * (intensityMax - intensityMin + 1) + intensityMin
    );
  } else {
  ...
  attackOrHeal === 1
    ? changingAudio('Audio/GameAudio/monsterAttack.mp3')
    : changingAudio('Audio/GameAudio/healing.mp3');
  attackOrHeal === 1
    ? (userHealthBar.value -= value)
    : (monsterHealthBar.value += value); // Monster attacks User or Heals
  attackOrHeal === 1
    ? (userHealthNumber.innerText = userHealthBar.value)
    : (monsterHealthNumber.innerText = monsterHealthBar.value);
  ...
  setTimeout(() => {
    //the reason this setTimeout is here is because we need 2 seconds to show the round Context header as without this, it'll remove the context Header without information being displayed
    if (monsterCounter !== 2 || userHealthBar.value !== 0) {
      // only execute if MonsterCounter is not = 2, and userhealthBar is not = 0
...
function healthBarChecker(healthBarElement) {
  ...
  if (healthBarElement.value > 0) {
    // if User Or Monster's health is not 0, function will finish executing
    healthBarTransitionColor(healthBarElement);
    return;
  } else if (healthBarElement.id === 'monsterHealthBar') {
    // Monster is dead
    userCounter++;
  ...
  if (gameWinner !== 'Pending') {
    // If User Or Monster has won 2 rounds, this code will execute and stop the function from executing further
    alert(alertMessages[gameWinner]);
    roundContextHeader.style.display = 'block';
    roundContextHeader.innerText =
      gameWinner === 0 ? 'User has Won' : 'Monster has Won'; // code for crown to appear on Winner
...
function healthBarTransitionColor(healthBarElement) {
  if (healthBarElement === 'newRound') {
    // code for when new round commences, both monster and user health needs to be back to color Green!
  ...
  } else if (healthBarElement.value >= 67 && healthBarElement.value <= 100) {
    // Code Changing User Or Monster health bar to Green
    healthBarElement.classList.remove('yellowHealthBar');
    healthBarElement.classList.add('greenHealthBar');
  } else if (healthBarElement.value >= 34 && healthBarElement.value <= 67) {
    // Code Changing User Or Monster health bar to Yellow
